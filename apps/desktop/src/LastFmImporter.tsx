...
import type { LastFmImportDefaults, LastFmImportState, MetadataValues, Settings } from './types.ts'
import { activeImportQueue, applyCurrentImportPageResponse, automaticCollectionAlbumContributors, canHandleImportShortcut, collectionAlbumActionLabel, collectionAlbumTrackStatuses, collectionAmbiguousChoices, collectionCoverageStatus, collectionDialogInitialState, collectionDialogScreen, collectionDialogTransition, collectionPreviewCoverageCopy, collectionSuggestion, downloadAction, excludeImportRows, excludedImportCount, handleImportQueueTab, importAlbumActionAdvances, importCountMergePresentation, importDownloadCopy, importDownloadPercent, importEmptyPageMessage, importQueueHighlightIndex, importQueueTabTarget, importQueueVisibleRange, importStatusText, isCurrentImportPageResponse, loadSelectedImportPage, moveImportNavigationRow, moveImportQueueIndex, nextRemainingImportQueue, pickerCandidates, pickerSelectedUri, projectAcknowledgedImportApply, projectImportQueueExclusion, requiredImportMatchIds, restPendingImportCount, selectImportRows, selectedCollectionAlbumUris, selectedImportCount, selectedImportTrackConfidence, setWholeAlbumImport, shouldRefreshImportEvent, showsImportRemaining, sortImportQueue, spotifyLimitCountdown, stablePartitionImportRows, strongImportAlbumMatch, toggleImportRow, trackPickerQuery, validImportIntent, type CollectionAmbiguousChoice, type CollectionTrackStatusProjection, type CountMode, type ImportConfidence, type ImportNavigationTarget, type ImportPickerKind, type ImportQueueItem, type ImportQueuePage, type ImportQueueTabTarget, type ImportRowSelection, type ImportSourceRow, type ReviewState } from './lastfmImportState.ts'
...
function ImportPage({ page, failed, showQueries, onRefresh, onRejected, onNext, onApplied, onPrevious, onError, onCollectionPage, onTabToQueue, onShortcuts, onStatus, onMutation }: { page: PageView; failed: boolean; showQueries: boolean; onRefresh: (strict?: boolean) => Promise<ImportQueueItem[]>; onRejected: (page: PageView, remainingPlayCount: number, allExcluded: boolean) => ImportQueueItem[]; onNext: (queue?: ImportQueueItem[], focusQueue?: boolean) => void; onApplied: () => Promise<void>; onPrevious: () => void; onError: (error: unknown) => void; onCollectionPage: (page: PageView) => void; onTabToQueue: () => boolean; onShortcuts: () => void; onStatus: ShortcutStatus; onMutation: (running: boolean) => void }) {
  ...
  useEffect(() => {
    let active = true
    invoke<MetadataValues>('metadata_values').then((values) => { if (active) setGenreSuggestions(values.cats) }).catch((error) => { if (active) onError(error) })
    return () => { active = false }
  }, [onError])
  const saveOptions = (next: ReviewState) => invoke('lastfm_import_options', { batchId: page.batchId, artist: page.artist, album: page.album, options: pageOptions(next) })
  ...
  const persistGenre = () => { void saveOptions(review).catch(onError) }
  ...
  const flushRejectedRows = async () => {
    ...
    try {
      while (queuedRejects.current.exclude.size || queuedRejects.current.restore.size) {
        ...
        const ids = [...pending]
        pending.clear()
        nextState = await invoke<ImportStateView>('lastfm_import_review', { batchId: page.batchId, ids, action, artist: page.artist, album: page.album })
      }
  ...
  const activateCollection = async () => {
    const next = await runPageMutation('lastfm_import_activate_collection', { batchId: page.batchId, artist: page.artist, album: page.album })
  ...
  const handleNavigationKeyDown = (event: ReactKeyboardEvent<HTMLElement>) => {
    ...
    if (event.key === ' ') {
      if (row === 0) {
        if (convertedCollection) onStatus('Whole-album import is unavailable after switching to album matches.')
  ...
  const convertedCollection = collection && page.album !== ''
  ...
  const automaticCollectionAlbums = automaticCollectionAlbumContributors(
    collection ? projectedRows.filter((item) => !item.decision.excluded).map(matchedTrack).filter((track) => track !== null) : [],
  ...
  const wholeAlbumDisabled = convertedCollection || busy || failed || !review.importContent || !collectionReady
  ...
  return <div ref={list} className="import-queue-list" aria-label="Import queue">
    <div className="import-queue-canvas" style={{ height: range.contentHeight }}>
      <div className="import-queue-window" style={{ transform: `translateY(${range.offsetTop}px)` }}>
        {items.slice(range.start, range.end).map((item, index) => { const absoluteIndex = range.start + index; return <button type="button" data-import-nav="queue" data-queue-index={absoluteIndex} data-highlighted={highlightedIndex === absoluteIndex ? 'true' : undefined} aria-current={selectedPage === item.page ? 'true' : undefined} aria-keyshortcuts="ArrowUp ArrowDown Tab Shift+Tab Enter S ?" tabIndex={highlightedIndex === absoluteIndex ? 0 : -1} aria-label={`Batch ${absoluteIndex + 1} of ${items.length}: ${item.album || 'Singles'} by ${item.artist}, ${item.remainingPlayCount.toLocaleString()} plays to import${item.importedPlayCount ? `, ${item.importedPlayCount.toLocaleString()} plays already imported` : ''}`} disabled={disabled} className={`import-queue-row${selectedPage === item.page ? ' selected' : ''}${highlightedIndex === absoluteIndex ? ' highlighted' : ''}`} key={item.page} onFocus={() => { highlightedPageRef.current = item.page; setHighlightedIndex((current) => current === absoluteIndex ? current : absoluteIndex) }} onKeyDown={(event) => handleKeyDown(event, item, absoluteIndex)} onClick={() => { highlightedPageRef.current = item.page; setHighlightedIndex(absoluteIndex); onOpen(item) }}><span className={`import-status-dot ${item.status ?? 'pending'}`} aria-label={item.status ?? 'pending'} title={item.error ?? undefined}>{item.status === 'done' ? '✓' : item.status === 'skipped' ? '–' : item.status === 'failed' ? '!' : item.status === 'excluded' || item.status?.startsWith('ignored') ? '⊘' : '•'}</span><span className="import-queue-copy"><strong>{item.album || 'Singles'}</strong><small>{item.error ? `Apply failed: ${item.error}` : `${item.artist} · ${item.sourceCount} tracks`}</small></span><span className="import-queue-count"><span className="plays-to-import">{item.remainingPlayCount.toLocaleString()} <small>plays to import</small></span>{item.importedPlayCount > 0 && <span className="plays-imported">{item.importedPlayCount.toLocaleString()} <small>plays imported</small></span>}</span></button> })}
      </div>
    </div>
  </div>
}
...
export default function LastFmImporter() {
  ...
  const acknowledgeReviewExclusion = (nextPage: PageView, remainingPlayCount: number, allExcluded: boolean) => {
    ...
    setQueue(nextQueue)
    setPage(pageWithQueuePosition(nextPage, activeImportQueue(sortImportQueue(nextQueue, sort))))
    return nextQueue
  }
