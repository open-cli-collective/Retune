...
function ImporterRow({ item, rowNumber, checked, selected, needsMatch, collection, selectedTrackUris, ambiguousChoices, fuzzy, fuzzyTarget, fuzzyResultCount, onToggle, onExclude, onSelect, onChangeTrack, onUseTrack, onFuzzyMode, onFuzzyToggle, fuzzyExpanded, fuzzyMode, fuzzyLocked, showQuery, locked }: { item: PageItem; rowNumber: number; checked: boolean; selected: boolean; needsMatch: boolean; collection: boolean; selectedTrackUris: Iterable<string>; ambiguousChoices: CollectionAmbiguousChoice[]; fuzzy?: ImportSourceRow[]; fuzzyTarget?: string; fuzzyResultCount: number; onToggle: () => void; onExclude: () => void; onSelect: (event: ReactMouseEvent<HTMLDivElement>) => void; onChangeTrack: () => void; onUseTrack: (uri: string) => void; onFuzzyMode: (mode: CountMode) => void; onFuzzyToggle: () => void; fuzzyExpanded: boolean; fuzzyMode: CountMode; fuzzyLocked: boolean; showQuery: boolean; locked: boolean }) {
  ...
  return <article className={`import-track-row${excluded ? ' excluded' : ''}${selected ? ' selected' : ''}`}>
...
function ImportPage({ page, failed, showQueries, onRefresh, onNext, onApplied, onPrevious, onError, onCollectionPage, onTabToQueue, onShortcuts, onStatus, onMutation }: { page: PageView; failed: boolean; showQueries: boolean; onRefresh: (strict?: boolean) => Promise<ImportQueueItem[]>; onNext: (queue?: ImportQueueItem[], focusQueue?: boolean) => void; onApplied: () => Promise<void>; onPrevious: () => void; onError: (error: unknown) => void; onCollectionPage: (page: PageView) => void; onTabToQueue: () => boolean; onShortcuts: () => void; onStatus: ShortcutStatus; onMutation: (running: boolean) => void }) {
  ...
  const rejectRows = async (ids: Iterable<string>, excluded: boolean) => {
    const rowIds = [...new Set(ids)]
    if (!rowIds.length || busy) return
    setReview((current) => excludeImportRows(current, rowIds, excluded))
    ...
    try {
      try {
        await invoke('lastfm_import_review', { batchId: page.batchId, ids: rowIds, action: excluded ? 'exclude' : 'undo-exclude', artist: page.artist, album: page.album })
      } catch (error) {
        mutationError = error
      }
      try {
      ...
      if (mutationError) {
        setPageError(String(mutationError))
