...
const reviewState = (rows: ImportSourceRow[]): ReviewState => ({
  rows,
  decisions: Object.fromEntries(rows.map((row) => [row.stableId, { status: 'pending' as const }])),
  importContent: true,
...
test('Last.fm row selection supports plain, Shift, Cmd, and Ctrl gestures', () => {
  const visible = ['required', 'ordinary', 'other', 'rejected']
  let selection = selectImportRows(visible, [], null, 'ordinary', {})
  assert.deepEqual([...selection.ids], ['ordinary'])
  assert.equal(selection.anchor, 'ordinary')
  selection = selectImportRows(visible, selection.ids, selection.anchor, 'required', { shiftKey: true })
  assert.deepEqual([...selection.ids], ['required', 'ordinary'])
  assert.equal(selection.anchor, 'ordinary')
  selection = selectImportRows(visible, selection.ids, selection.anchor, 'rejected', { metaKey: true })
  assert.deepEqual([...selection.ids], ['required', 'ordinary', 'rejected'])
  assert.equal(selection.anchor, 'rejected')
  selection = selectImportRows(visible, selection.ids, selection.anchor, 'rejected', { ctrlKey: true })
  assert.deepEqual([...selection.ids], ['required', 'ordinary'])
})
...
test('Last.fm play sorting prioritizes remaining work then source track count', () => {
  const queue = importQueue()
  queue[0] = { ...queue[0], playCount: 4_000, remainingPlayCount: 92, sourceCount: 55 }
  queue[1] = { ...queue[1], playCount: 760, remainingPlayCount: 122, sourceCount: 45 }
  assert.deepEqual(sortImportQueue(queue, 'plays').map((item) => item.artist), ['Alpha', 'Beta'])

  queue[0] = { ...queue[0], remainingPlayCount: 122, sourceCount: 55 }
  assert.deepEqual(sortImportQueue(queue, 'plays').map((item) => item.artist), ['Beta', 'Alpha'])
})
...
test('Last.fm acknowledged apply projects the next queue choice before authoritative refresh', () => {
  ...
  const archived = projectAcknowledgedImportApply(importQueue(), 1, 'batch', true)
  assert.equal(archived.queue[0].archived, true)
})
...
test('Last.fm strong album match advisory accepts exact and supersets but rejects unsafe mappings', () => {
  ...
  assert.deepEqual(strongImportAlbumMatch('the-artist', exact, [
    { targetUri: 'spotify:track:one' },
    { targetUri: 'spotify:track:two' },
  ...
  assert.equal(strongImportAlbumMatch('The Artist', exact, [
    { targetUri: 'spotify:track:standalone', standaloneLowConfidence: true },
    { targetUri: null },
  ]).strong, false)
...
test('Last.fm collection suggestions require one eligible same-artist candidate', () => {
  const source = { stableId: 'source', artist: 'Artist', variants: [] }
  const suggest = (candidates: Array<{ uri: string; artist: string; inLibrary: boolean; relation: 'best-match' | 'same-songs' | 'superset' | null }>, selectedTrackUris: string[] = []) => collectionSuggestion(source, { selectedUri: null, trackMatches: {}, candidates }, selectedTrackUris)
  ...
  assert.equal(suggest([ownedNear, { ...ownedNear, uri: 'spotify:track:other' }]), null)
  assert.deepEqual(suggest([ownedNear, { ...ownedNear }]), ownedNear)
})

test('Last.fm visible rows keep required work first with stable source order', () => {
  const rows = importRows()
  assert.deepEqual(stablePartitionImportRows(rows, ['b'], (row) => row.stableId).map((row) => row.stableId), ['b', 'a'])
  assert.deepEqual(stablePartitionImportRows(rows, [], (row) => row.stableId).map((row) => row.stableId), ['a', 'b'])
})

test('Last.fm row sorts keep unmapped work first', () => {
  const rows = importRows().map((source) => ({ source }))
  assert.deepEqual(sortImportRows(rows, ['b'], 'plays').map((row) => row.source.stableId), ['b', 'a'])
  assert.deepEqual(sortImportRows(rows, [], 'plays').map((row) => row.source.stableId), ['a', 'b'])
  assert.deepEqual(sortImportRows(rows, [], 'lastPlayed').map((row) => row.source.stableId), ['b', 'a'])
  assert.deepEqual(sortImportRows(rows, [], 'track').map((row) => row.source.stableId), ['a', 'b'])
  assert.deepEqual(sortImportRows(rows, [], 'source').map((row) => row.source.stableId), ['a', 'b'])
})

test('Last.fm queue filter matches all scalar batch data across search terms', () => {
  const queue = [
    { page: 1, artist: 'Linkin Park', album: 'Hybrid Theory', name: 'Night at the Opera', playCount: 724, importedPlayCount: 0, remainingPlayCount: 724, latest: 30, sourceCount: 14, remaining: true, albumEntities: 1, trackEntities: 0, status: 'failed' as const, error: 'Spotify quota' },
  ...
  assert.equal(filterImportQueue(queue, 'night opera')[0]?.page, 1)
...
test('Last.fm collection dialog preserves results after preview Add and on failures', () => {
  ...
  const results = [{ uri: 'spotify:album:a' }, { uri: 'spotify:album:b' }]
  const cachedSelected = ['spotify:album:b']
  const selected = () => selectedCollectionAlbumUris(cached, cachedSelected)
  let state = collectionDialogInitialState<{ uri: string }>()
  state = collectionDialogTransition(state, { type: 'set-query', query: 'Artist album' })
  state = collectionDialogTransition(state, { type: 'search-succeeded', results })
  ...
  assert.equal(state.resultsScrollTop, 240)

  let firstPreviewState = collectionDialogInitialState<{ uri: string }>()
  firstPreviewState = collectionDialogTransition(firstPreviewState, { type: 'set-query', query: 'Artist album' })
  firstPreviewState = collectionDialogTransition(firstPreviewState, { type: 'search-succeeded', results })
  firstPreviewState = collectionDialogTransition(firstPreviewState, { type: 'preview-started', uri: 'spotify:album:a', resultsScrollTop: 240 })
  firstPreviewState = collectionDialogTransition(firstPreviewState, { type: 'preview-failed' })
  assert.equal(firstPreviewState.query, 'Artist album')
  assert.deepEqual(firstPreviewState.results, results)
  assert.equal(firstPreviewState.previewUri, undefined)
  assert.equal(firstPreviewState.resultsScrollTop, 240)

  assert.deepEqual(collectionDialogTransition(state, { type: 'search-failed' }), state)
  const failedPreview = collectionDialogTransition(
    collectionDialogTransition(state, { type: 'preview-started', uri: 'spotify:album:b', resultsScrollTop: 320 }),
    { type: 'preview-failed' },
  )
  assert.equal(failedPreview.query, state.query)
  assert.deepEqual(failedPreview.results, state.results)
...
test('Last.fm setup and download status reflect connected and active states', () => {
  ...
  assert.deepEqual(importEmptyPageMessage('review', true), { title: 'Matching this review batch…', detail: 'Loading and ranking likely matches.' })
...
test('column and browser resizing preserve usable minimums', () => {
  ...
  assert.equal(resizedColumnWidth(230, 100, 100, 1.15), 200)
  assert.equal(resizedColumnWidth(230, 100, 111.5, 1.15), 210)
...
test('same-batch importer refresh preserves draft options while accepting authoritative rows', () => {
  ...
  const incoming = { ...reviewState(rows), decisions: { a: { status: 'done' as const }, b: { status: 'pending' as const } } }
