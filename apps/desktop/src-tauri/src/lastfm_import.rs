...
impl Service {
    ...
    async fn set_match(&self, result: MatchResult) -> Result<(), String> {
        self.set_matches(vec![result]).await
    }
...
async fn apply_page(
    ...
    service
        .commit_rows(&committed, artist, album, options)
        .await
}
...
pub(crate) async fn lastfm_import_review(
    ...
    state
        .lastfm_import
        .review_action(&id, &action, &artist, &album)
        .await?;
...
pub(crate) async fn lastfm_import_options(
    ...
    state
        .lastfm_import
        .update_options(&artist, &album, options)
        .await?;
    emit_import_changed(&app, state.lastfm_import.as_ref()).await
}
...
pub(crate) async fn lastfm_import_count_mode(
    ...
    state
        .lastfm_import
        .set_count_mode(&target_uri, mode)
        .await?;
    emit_import_changed(&app, state.lastfm_import.as_ref()).await
}
...
pub(crate) async fn lastfm_import_search_terms(
    ...
    let state = app.state::<crate::AppState>();
    state.lastfm_import.set_search_terms(show).await?;
    emit_import_changed(&app, state.lastfm_import.as_ref()).await
}
...
pub(crate) async fn lastfm_import_select_match(
    ...
    state.lastfm_import.select_match(&id, &uri).await?;
    emit_import_changed(&app, state.lastfm_import.as_ref()).await
}
...
pub(crate) async fn lastfm_import_change_album(
    ...
    let matches = session
        ...
        .collect();
    state.lastfm_import.set_matches(matches).await?;
    emit_import_changed(&app, state.lastfm_import.as_ref()).await
}
    ...
    for item in &page.rows {
        ...
        if candidate.uri.starts_with("spotify:album:") {
            if selected_album_uris.insert(candidate.uri.clone()) {
                state
                    .lastfm_import
                    .select_match(&item.source.stable_id, &candidate.uri)
                    .await?;
            }
        } else {
            state
...
mod tests {
    ...
    async fn count_mode_change_is_rejected_after_target_is_done() {
        ...
        assert!(service
            .set_count_mode(target, CountMode::Overwrite)
            .await
            .is_ok());
        assert!(service
            .set_count_mode(target, CountMode::Zero)
            .await
            .is_err());
        assert_eq!(
    ...
    async fn fully_excluded_review_action_reaches_done_and_has_view_only_queue_status() {
        ...
        for row in &session.rows {
            service
                .review_action(&row.stable_id, "exclude", "A", "Album")
                .await
                .unwrap();
        }
    ...
    async fn search_terms_preference_round_trips_on_resume() {
        ...
        service
            ...
            .await
            .unwrap();
        service.set_search_terms(false).await.unwrap();
    ...
    async fn selecting_an_album_candidate_remaps_every_related_source_track() {
        ...
        service
            .select_match(&rows[0].stable_id, "spotify:album:new")
            .await
            .unwrap();
        let session = service.snapshot().await.unwrap();
        for row in rows {
